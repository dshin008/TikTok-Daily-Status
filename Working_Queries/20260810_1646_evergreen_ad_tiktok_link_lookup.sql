-- Evergreen VSA Broad Web: ad_id → creative_id → tiktok_link lookup
-- Purpose: replace broken Performance Tracker TT Link (col F) formulas
-- Join key: creative_id from visit landing URLs → creative_assets_id_masterlist
-- Updated: 2026-08-10
...
WITH evergreen_ads AS (
  SELECT
    ...
    ROUND(SUM(spend), 2) AS spend_14d
  ...
  WHERE accountID = '7125498373565726721'
    ...
    AND reportDate >= DATE_SUB(CURRENT_DATE(), INTERVAL 14 DAY)
  GROUP BY 1
),
...
creative_from_visits AS (
  ...
  WHERE cmcode = 'TT49VSA'
    AND SessionStartDate >= DATE_SUB(CURRENT_DATE('America/New_York'), INTERVAL 40 DAY)
    AND event_soid = 49
...
-- One TikTok URL per Creative Asset ID (masterlist can have duplicate IDs)
...
LEFT JOIN creative_from_visits v
  ON a.ad_id = v.ad_id
LEFT JOIN masterlist_deduped m
  ON v.creative_id = m.Creative_Asset_ID
ORDER BY a.adgroup_name, a.spend_14d DESC
