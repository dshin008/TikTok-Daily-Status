-- Morning checkup: recent 3 settled days 2026-08-07 to 2026-08-09, daily grain
WITH visits AS (
  SELECT
  ...
  WHERE cmcode IN ('TT49BTSUF','TT49BTSMF','TT49NBAU','TT49VSA','TT49DPA','TT49GTMMF',
    ...
    AND sessionstartdate BETWEEN DATE('2026-08-07') AND DATE('2026-08-09')
    AND event_soid = 49
  GROUP BY ALL
)
SELECT d AS date, agid AS adgroup_id, SUM(v) AS visits, SUM(a) AS ATCs,
  SAFE_DIVIDE(SUM(a), SUM(v)) AS atc_rate, SUM(p) AS PDP_views,
  SAFE_DIVIDE(SUM(p), SUM(v)) AS pdp_rate
FROM visits WHERE agid IS NOT NULL AND agid != ''
GROUP BY 1, 2 ORDER BY adgroup_id, date
