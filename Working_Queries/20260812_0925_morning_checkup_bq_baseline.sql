-- Morning checkup: 14d baseline aggregate 2026-07-25..2026-08-07
WITH visits AS (
  SELECT
  ...
  WHERE cmcode IN ('TT49BTSUF','TT49BTSMF','TT49NBAU','TT49VSA','TT49DPA','TT49GTMMF',
               ...
               'TT49RMN')
    AND sessionstartdate BETWEEN DATE('2026-07-25') AND DATE('2026-08-07')
    AND event_soid = 49
  GROUP BY ALL
),
tiktok_lookup AS (
  ...
  WHERE accountID = "7125498373565726721"
    AND reportDate >= DATE('2026-07-25') - 10
...
SELECT
  ...
  SAFE_DIVIDE(SUM(v.ATCs), SUM(v.PDP_views)) AS vtc_rate
FROM visits v
