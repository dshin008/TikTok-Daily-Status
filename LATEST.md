...
2026-08-12 · Yesterday = 2026-08-11 · BigQuery settled through 2026-08-10

HEADLINE: 16 ad groups need a check today; $10,684 spent there yesterday (account spend flat vs 7d avg). Evergreen VSA LF/FF still cluster the Low-funnel flags — chairs, shoe storage, bedroom furniture, and couches show ATC and/or VTC damage; several Mid VC groups are on a 2-day CTR collapse.

Account pulse (yesterday vs 7-day daily avg, TikTok-native)
  Spend        $48,140     vs avg $47,812     ▲ 1%
  CTR          1.11%      vs avg 1.23%     ▼ 10%
  By funnel: Upper 92 groups · Mid 169 groups · Low 417 groups (3 excluded: LEAD_GENERATION; 6 unresolved WEB_CONVERSIONS)

Campaign clustering
  · 4 of today's flags are on Evergreen VSA Broad Web LF — likely one shared cause (creative, landing page, or a campaign-level setting change), not 4 separate issues.
  · 4 of today's flags are on Evergreen VSA Broad Web FF — likely one shared cause (creative, landing page, or a campaign-level setting change), not 4 separate issues.
...
🔴 Check today (16 ad groups, showing top 10 by spend)
...
Notes for creative associate (if any)
  ...
  · verified_creator_boosted_vca_FF — possible creative-driven CTR decay; not a rotation plan.
  · TT_BTS_CLP_VC_FF — possible creative-driven CTR decay; not a rotation plan.
  · Tailgating_Internal_VC_FF — possible creative-driven CTR decay; not a rotation plan.

Assumptions & limits
  · TikTok windows same-day fresh; BigQuery windows settled through 2026-08-10 (revenue/visits lag).
  · Funnel-aware KPIs: UF=6s VTR, MF=PDP Rate, LF=ATC Rate. LEAD_GENERATION campaigns excluded.
  · Flags fire on decay (vs own baseline), yesterday anomaly (2-day persistence), sustained low vs funnel peers, or a PDP-to-cart (VTC) drop.
  · Signal E (ad-level CTR/ATC gap) only runs on 🔴 groups.
  · This skill changes nothing in Ads Manager or BigQuery.

Do next (prioritized)
  1. [Today] Spark_Chairs_Broad_LF — ATC + VTC both 3/3 below baseline/peers; open destination URLs first (product match + stock), then delivery/budget.
  2. [Today] Spark_ShoeStorage_Broad_LF + Spark_BedroomFurniture_Broad_LF/FF — VTC/link-risk pattern; check product links before budget cuts.
  3. [Today] Halloween_Creator_VC_FF / verified_creator_boosted_vca_FF / TT_BTS_CLP_VC_FF — 2-day CTR collapse; confirm creative fatigue vs auction pressure.
  4. [Today] CrashTest_Q3_VCA_FF — spend drop + PDP collapse vs a very high 14d baseline (possible tracking/landing change).
  5. Watch: Spark_Bed_Broad_LF/FF still on VTC soft (2/3) — keep monitoring post prior link fix.
  · `vtc-decay` / `low-vtc` / `ad-link-risk` groups → check destination URL(s) first before budget, bid, or creative
